 ...
 *
 * The function \ref ep_test_data_is_compatible_endpoint can be used to check
...
#ifndef ENDPOINTTESTDATA_H_INCLUDED
#define ENDPOINTTESTDATA_H_INCLUDED
 ...
 * \ref ep_test_data_get_frame_data depends on the radar sensor hardware.
 *
...
typedef enum
{
    EP_TEST_DATA_RX_DATA_REAL                = 0, /**< The frame data
                                                        contains only I or Q
                                                        signal. */
    EP_TEST_DATA_RX_DATA_COMPLEX             = 1, /**< The frame data
                                                        contains I and Q
                                                        signals in separate
                                                        data blocks. */
    EP_TEST_DATA_RX_DATA_COMPLEX_INTERLEAVED = 2  /**< The frame data
                                                        contains I and Q
                                                        signals in one
                                                        interleaved data
                                                        block. */
...
typedef enum
{
    EP_TEST_DATA_SIGNAL_ONLY_I  = 0, /**< Only the I signal is captured
                                           during radar data frame
                                           acquisition. */
    EP_TEST_DATA_SIGNAL_ONLY_Q  = 1, /**< Only the Q signal is captured
                                           during radar data frame
                                           acquisition. */
    EP_TEST_DATA_SIGNAL_I_AND_Q = 2  /**< Both, I and Q signal are captured
 ...
 *        point.
 *
 * Use \ref ep_test_data_get_driver_version to query the driver version
 ...
 *        capabilities.
 *
 * Use \ref ep_test_data_get_device_info to query the device information. When the
 ...
 * \ref ep_test_data_set_frame_format to change the frame format. When the
 ...
 * \ref ep_test_data_get_frame_format to query the current RF settings.
 */
typedef struct
{
 ...
 * \ref callback_data_frame_test_t when \ref ep_test_data_get_frame_data is
 * called.
 *
...
typedef struct
{
    ...
    uint32_t         frame_number;           /**< The running number of the
                                                  ...
                                                  \ref ep_test_data_set_automatic_frame_trigger
 ...
 * function by calling \ref ep_test_data_set_callback_data_frame_test.
 *
 ...
 *                             \ref ep_test_data_set_callback_data_frame_test.
 ...
 */
typedef void(*callback_data_frame_test_Test_t)(void* context, int32_t protocol_handle,
                                     uint8_t endpoint,
 ...
 * \ref ep_test_data_set_callback_driver_version.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 ...
 * callback of this type is issued. The user must register the callback
 * function by calling \ref ep_test_data_set_callback_device_info.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 * \param[in] context          The context data pointer, provided along with
 *                             the callback itself through
 *                             \ref ep_test_data_set_callback_device_info.
 * \param[in] protocol_handle  The handle of the connection, the sending
 *                             device is connected to.
 * \param[in] endpoint         The number of the endpoint that has sent the
 *                             message.
 ...
 * callback of this type is issued. The user must register the callback
 * function by calling \ref ep_test_data_set_callback_frame_format.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 * \param[in] context          The context data pointer, provided along with
 *                             the callback itself through
 *                             \ref ep_test_data_set_callback_frame_format.
 * \param[in] protocol_handle  The handle of the connection, the sending
 *                             device is connected to.
 * \param[in] endpoint         The number of the endpoint that has sent the
 *                             message.
 ...
 * callback of this type is issued. The user must register the callback
 * function by calling \ref ep_test_data_set_callback_temperature.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 * \param[in] context           The context data pointer, provided along with
 *                              the callback itself through
 *                              \ref ep_test_data_set_callback_temperature.
 * \param[in] protocol_handle   The handle of the connection, the sending
 *                              device is connected to.
 * \param[in] endpoint          The number of the endpoint that has sent the
 *                              message.
 ...
 * callback function by calling \ref ep_test_data_set_callback_tx_power.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 * \param[in] context          The context data pointer, provided along with
 *                             the callback itself through
 *                             \ref ep_test_data_set_callback_tx_power.
 * \param[in] protocol_handle  The handle of the connection, the sending
 *                             device is connected to.
 * \param[in] endpoint         The number of the endpoint that has sent the
 *                             message.
 ...
 * callback of this type is issued. The user must register the callback
 * function by calling \ref ep_test_data_set_callback_chirp_duration.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 * \param[in] context            The context data pointer, provided along with
 *                               the callback itself through
 *                               \ref ep_test_data_set_callback_chirp_duration.
 * \param[in] protocol_handle    The handle of the connection, the sending
 *                               device is connected to.
 * \param[in] endpoint           The number of the endpoint that has sent the
 *                               message.
 ...
 * callback function by calling
 * \ref ep_test_data_set_callback_min_frame_interval.
 *
 * Whenever a callback is issued, the following parameters are passed to the
 * callback:
 *
 ...
 * \ref callback_data_frame_test_t.
 *
 ...
 *                      function.
 */
void ep_test_data_set_callback_data_frame_test(callback_data_frame_test_Test_t callback,
                                           void* context);
 ...
 * immediately with status code EP_TEST_DATA_STATUS_TIME_OUT if no data is
 * available.
 *
 ...
 * type \ref callback_data_frame_test_t is issued.
 *
 * \param[in] protocol_handle  A handle to the connection to the sensor
 *                             device.
 ...
 * \ref ep_test_data_get_frame_data.
 *
 * The function waits for the status code returned by the device and returns
 * it.
 *
 ...
 * \ref ep_test_data_get_device_info. The number of the first temperature
 * sensor is 0.
 *
 * \param[in] protocol_handle  A handle to the connection to the sensor
 *                             device.
 ...
 * \ref ep_test_data_get_device_info. The number of the first antenna (and TX
 * power sensor) is 0.
 *
 * \param[in] protocol_handle  A handle to the connection to the sensor
 *                             device.
