/*
 ...
 @brief: This file defines the driver API for Position2Go Board of
...
typedef struct
{
    ...
    uint32_t up_chirp_duration_nsec; /**<  The duration of the up-chirp */
    uint32_t down_chirp_duration_nsec; /**< The duration of the down-chirp */
    uint32_t chirp_to_chirp_delay_nsec; /** The complete duration of a chirp */
...
typedef struct {
	uint32_t  			fft_window_size; 		/**< the fft window size, used by signal processing on time domain signals */
	fft_window_type_t  	window_type; 			/**< the fft window type, used by signal processing on time domain signals */
	uint8_t				mean_removed;			/**< set to 1 if the mean value is removed by processing */
	float 				scaling_factor;			/**< the factor to scale the fft values */
...
/**
 * \brief This function returns the fft configuration that is used on the device
 *
 * \param[in] device  A handle to the radar sensor device.
 * \param[in] device  pointer to fft_configuration structure
 *
 * \return	Unsigned 16-bit integer, 0 for false (disabled) and non-zero for true (enabled) case.
 */
